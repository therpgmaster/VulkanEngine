...
public:
	...
	glm::mat4 mat4()
	{
		return  makeMatrix(rotation, scale, translation); 
				// * makeMatrix(localRotation, { 1.f, 1.f, 1.f }, localTranslation);
	}
	...
	glm::vec3 getForwardVector() { return { sin(rotation.y), 0.f, cos(rotation.y) }; }
	glm::vec3 getRightVector() 
	{ 
		const auto fwd = getForwardVector();
		return { fwd.z, 0.f, -fwd.x }; 
	}
	// TODO: currently returns global Z+ = up
	glm::vec3 getUpVector() { return { 0.f, 0.f, 1.f }; }
